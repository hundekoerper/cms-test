...
router.post('/pages/delete/:id', sessionCheck, function(request, response){
  var id = request.body._id;
...
router.post('/add-user', function(request, response){
  ...
  AdminUser.save(function(err){
    if(!err){
      return response.send('Admin User added!')
    } else {
...
router.post('/login', function(request, response){
  ...
  adminUser.find({
    ...
    if(err | data === null){
      return response.send(401, 'User not found');
    } else {
      var usr = data;
      ...
      if (username == usr.username && bcrypt.compareSync(password, usr.password)) {
        ...
        request.session.regenerate(function(){
          ...
          return request.send(username);
        });

      } else {
        return response.send(401, 'Wrong Username or Password');
      }
    }
  });
});
